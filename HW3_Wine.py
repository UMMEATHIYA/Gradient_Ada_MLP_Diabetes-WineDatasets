...
if feat_select==1:
    ...
    if fs_type==2:
        ...
        if binning==0:
            rgr = GradientBoostingRegressor(n_estimators=100, loss='squared_error', learning_rate=0.1, max_depth=3, min_samples_split=3, random_state=rand_st)
...
if binning==0 and cross_val==1:
    ...
    start_ts=time.time()
    rgr=AdaBoostRegressor(n_estimators=100, loss='linear', learning_rate=0.5, random_state=rand_st)
    scores=cross_validate(estimator=rgr, X=data_np, y=target_np, scoring=scorers, cv=5)                                                                                                  
    ...
    start_ts=time.time()
    clf = MLPRegressor(activation='logistic', solver='lbfgs', alpha=0.0001, max_iter=1000, hidden_layer_sizes=(10,),random_state=rand_st)
...
if binning==1 and cross_val==1:
    ...
    start_ts=time.time()
    clf=clf = AdaBoostClassifier(n_estimators=100, learning_rate=0.1, random_state=rand_st)
    scores=cross_validate(estimator=clf, X=data_np, y=target_np, scoring=scorers, cv=5)

    scores_Acc = scores['test_Accuracy']                                                                                                                                    
    print("AdaBoost Accuracy: %0.2f (+/- %0.2f)" % (scores_Acc.mean(), scores_Acc.std() * 2))                                                                                                    
    scores_AUC= scores['test_roc_auc']                                                                     #Only works with binary classes, not multiclass                  
    ...
    scores = cross_validate(estimator=clf, X=data_np, y=target_np, scoring=scorers, cv=5)
    scores_Acc = scores['test_Accuracy']                                                                                                                                    
    print("MLP Classifier Accuracy: %0.2f (+/- %0.2f)" % (scores_Acc.mean(), scores_Acc.std() * 2))                                                                                                    
    scores_AUC= scores['test_roc_auc']                                                                     #Only works with binary classes, not multiclass                  
